...
async function getProducts() {
  try {
    const r = await fetch(`https://api-seller.uzum.uz/api/seller-openapi/v1/product/shop/${SHOP_ID}?size=100&page=0`, {
      headers: { 'Authorization': UZUM_TOKEN }
    });
    if (!r.ok) return [];
...
async function sendReport(chatId) {
  ...
  const out = products.filter(p => (p.skuList?.[0]?.quantityActive || 0) === 0);
  const low = products.filter(p => { const q = p.skuList?.[0]?.quantityActive || 0; return q > 0 && q < 10; });

  let msg = '🚀 <b>Uzum Pro — Hisobot</b>\n\n';
  msg += `📦 Jami: <b>${products.length}</b> tovar\n`;
  msg += `✅ Sotuvda: <b>${products.length - out.length}</b>\n`;
  ...
  if (low.length > 0) {
    msg += '⚠️ <b>KAM QOLGAN:</b>\n';
    low.slice(0, 5).forEach(p => {
      const q = p.skuList?.[0]?.quantityActive || 0;
...
app.post('/webhook', (req, res) => { handleUpdate(req.body).catch(console.error); res.sendStatus(200); });

app.get('/api/shops', (req, res) => { res.json({ shops: [{ id: SHOP_ID, name: "Do'kon #" + SHOP_ID }] }); });
...
app.get('/api/products', async (req, res) => {
  try {
    ...
    const r = await fetch(`https://api-seller.uzum.uz/api/seller-openapi/v1/product/shop/${shopId}?size=${req.query.size||100}&page=${req.query.page||0}`, { headers: { 'Authorization': token } });
    if (!r.ok) { const t = await r.text(); return res.status(r.status).json({ error: 'HTTP ' + r.status, details: t }); }
    res.json(await r.json());
  } catch (err) { res.status(500).json({ error: err.message }); }
});

app.get('/api/health', (req, res) => { res.json({ status: 'ok', token: !!UZUM_TOKEN, bot: !!BOT_TOKEN, subscribers: global.subscribedChats.size }); });
