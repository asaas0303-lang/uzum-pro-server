...
bot.on('business_message', async (ctx) => {
  ...
  try {
    const from = msg.from;
    const language = ai.detectLanguage(msg.text || msg.caption || '');
    ...
    // 1) Mijoz RASM yuborgan bo'lsa — model aniqlaymiz
    if (msg.photo && msg.photo.length > 0) {
      await handleCustomerPhoto(ctx, msg, customer);
      return;
    }

    // 2) Mijoz OVOZLI XABAR yuborgan bo'lsa — matnga aylantirib javob beramiz
    if (msg.voice) {
      await handleCustomerVoice(ctx, msg, customer);
      return;
    }

    // 3) Mijoz MATN yozgan bo'lsa — AI orqali javob beramiz
    if (msg.text) {
...
// Mijoz kamera rasmi yuborganda: modelni aniqlab, video qo'llanma + qisqa
// tushuntirish yuboradi. Agar rasm xira/tushunarsiz bo'lsa, qayta so'raydi.
async function handleCustomerPhoto(ctx, msg, customer) {
  const models = db.getAllModels();
  const modelNames = Object.keys(models);
  if (modelNames.length === 0) return; // hali model qo'shilmagan
  ...
  const { modelName, isClear } = await ai.identifyModelFromImage({
    base64Image,
    mediaType,
    modelNames
  });

  const humanDelay = queueLib.randomDelay(25000, 70000); // ~25-70 soniya "o'ylab javob yozish"

  if (!isClear || !modelName) {
    const text = await ai.craftUnclearImageMessage({ language: customer.language });
    await queueLib.enqueue(
      () => sendBiz(ctx, msg, { text }),
  ...
  db.upsertCustomer({ chatId: msg.chat.id, lastModelName: modelName, businessConnectionId: msg.business_connection_id });

  await queueLib.enqueue(async () => {
    // Avval qisqa tabriklash/tasdiqlash matni
    const greeting = customer.language === 'ru'
      ? `Понял(а)! Это камера ${modelName}. Сейчас отправлю видео-инструкцию.`
      : `Tushundim! Bu — ${modelName} kamerasi. Hozir video qo'llanmani yuboraman.`;
    await sendBiz(ctx, msg, { text: greeting });

    if (model.videoGuides && model.videoGuides.length > 0) {
      await sendBiz(ctx, msg, { video: model.videoGuides[0].file_id });
    }
  }, { humanDelayMs: humanDelay });
}
...
async function handleCustomerText(ctx, msg, customer) {
  ...
  if (!model) {
    // Hali qaysi model ekani noma'lum — mehribonlik bilan rasm so'raymiz
    const text = customer.language === 'ru'
      ? 'Здравствуйте! Чтобы я мог точно помочь, отправьте, пожалуйста, фото вашей камеры 📷'
      : 'Assalomu alaykum! Sizga aniq yordam berishim uchun, iltimos, kameraning rasmini yuboring 📷';
    await queueLib.enqueue(() => sendBiz(ctx, msg, { text }), { humanDelayMs: humanDelay });
    return;
  }

  const answer = await ai.answerCustomerQuestion({
    question: msg.text,
    model,
  ...
  await queueLib.enqueue(() => sendBiz(ctx, msg, { text: answer }), { humanDelayMs: humanDelay });
}

async function handleCustomerVoice(ctx, msg, customer) {
  // Hozircha ovozli xabarni matnga aylantirish uchun tashqi xizmat ulanmagan.
  // Mijozga muloyimlik bilan matn yozishini so'raymiz.
...
// Business ulanish orqali xabar yuborish (matn/video/ovoz)
async function sendBiz(ctx, originalMsg, { text, video, voice }) {
  ...
  if (video) {
    await ctx.telegram.callApi('sendVideo', { ...base, video });
  }
  if (voice) {
