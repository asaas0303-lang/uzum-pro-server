...
async function handleCustomerText(ctx, msg, customer) {
  ...
  if (!model) {
    ...
    // Mijoz rahmat aytayotgan/yakunlayotgan bo'lsa — qayta rasm so'ramaymiz,
    // shunchaki iliq javob qaytaramiz.
    if (intent === 'gratitude') {
    ...
    // Mijoz rasm yubora olmasligini aytayotgan bo'lsa — qayta so'ramaymiz,
    // muammoni so'z bilan tushuntirishini so'raymiz.
    if (intent === 'cannot_send_photo') {
      const text = customer.language === 'ru'
        ? 'Хорошо, ничего страшного. Опишите, пожалуйста, словами, какая у вас модель камеры или какой вопрос возник.'
        : 'Tushunarli, hechqisi yo\'q. Kamerangiz qaysi model ekanini yoki qanday savolingiz borligini so\'z bilan yozib bersangiz bo\'ladi.';
      await queueLib.enqueue(() => sendBiz(ctx, msg, { text }), { humanDelayMs: humanDelay });
    ...
    // Agar avval allaqachon rasm so'ralgan bo'lsa — bir xil xabarni qayta
    // takrorlamaymiz, buning o'rniga savolga umumiy tarzda yordam berishga harakat qilamiz.
    if (customer.askedForPhotoOnce && intent === 'general') {
      const text = customer.language === 'ru'
        ? 'Расскажите, пожалуйста, подробнее о вашем вопросе — постараюсь помочь.'
        : 'Savolingizni biroz batafsilroq aytsangiz, yordam berishga harakat qilaman.';
      await queueLib.enqueue(() => sendBiz(ctx, msg, { text }), { humanDelayMs: humanDelay });
      db.upsertCustomer({ chatId: msg.chat.id, hasGreeted: true, businessConnectionId: msg.business_connection_id });
      return;
    }
    ...
    const text = customer.language === 'ru'
      ? (hasGreeted ? 'Чтобы я мог точно помочь, отправьте, пожалуйста, фото вашей камеры.' : 'Здравствуйте. Чтобы я мог точно помочь, отправьте, пожалуйста, фото вашей камеры.')
      : (hasGreeted ? 'Sizga aniq yordam berishim uchun, iltimos, kameraning rasmini yuboring.' : 'Assalomu alaykum. Sizga aniq yordam berishim uchun, iltimos, kameraning rasmini yuboring.');
    await queueLib.enqueue(() => sendBiz(ctx, msg, { text }), { humanDelayMs: humanDelay });
    db.upsertCustomer({ chatId: msg.chat.id, hasGreeted: true, askedForPhotoOnce: true, businessConnectionId: msg.business_connection_id });
    return;
  }
