...
// Model aniqlanmagan holda ham mijozning savoliga barcha mavjud
// kameralar uchun yozilgan umumiy ma'lumot asosida javob beradi.
// shouldAskForPhoto=true bo'lsa, javob oxirida bir marta rasm so'raydi.
async function answerGeneralCameraQuestion({ question, allModels, language, hasGreeted, shouldAskForPhoto }) {
  const allGuides = allModels.flatMap(m => [
    ...(m.textGuides || []).map(g => g.text),
    ...(m.manualImages || []).map(mi => mi.extractedText).filter(Boolean),
    ...(m.appScreenshots || []).map(a => a.extractedText).filter(Boolean)
  ]).filter(Boolean).join('\n---\n');

  const greetInstruction = hasGreeted
    ? 'Suhbat allaqachon boshlangan, QAYTA SALOM BERMA.'
    : 'Bitta marta samimiy salomlashish bilan boshla.';

  const photoInstruction = shouldAskForPhoto
    ? 'Javob oxirida, alohida qator bilan (### belgisi bilan ajratib), mijozdan kamerasining rasmini yuborishini iltimos qil — shunda aniqroq yordam bera olishingni ayt. Bu faqat bir marta bo\'lsin.'
    : '';

  const system =
    `Sen EnvoCam kamera do'konining samimiy yordamchisisan. Bot emassan, inson kabi yozasan. ` +
    `${NO_EMOJI_RULE} ` +
    `Javoblaring QISQA bo'lsin (har bir fikr 1-2 jumla). Agar bir nechta alohida fikr bo'lsa, "###" bilan ajrat. ` +
    `Mijozning tili: ${langName(language)}. ${greetInstruction} ` +
    `Quyida bizning WiFi kameralarimiz uchun mavjud umumiy ma'lumotlar berilgan. ` +
    `Ko'pchilik WiFi kameralar bir xil tartibda ishlaydi — shu ma'lumotlardan foydalanib, savolga real javob ber. ` +
    `Hech qachon "aniqroq ma'lumot yo'q" deb bo'sh javob berma — barcha mavjud bilimingdan foydalanib javob ber. ` +
    `${photoInstruction}\n\nUMUMIY MA'LUMOTLAR:\n${allGuides}`;
